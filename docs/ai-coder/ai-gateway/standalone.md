...
The components can connect when their [AI Gateway API versions are compatible](https://github.com/coder/coder/blob/main/coderd/aibridged/proto/version.go).
`coderd` checks API compatibility whenever a Gateway replica connects and rejects incompatible versions.
