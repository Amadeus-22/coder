...
func TestUntokenedProcessesReapSooner(t *testing.T) {
	...
	// 6 minutes is past exitedProcessReapAge (5m) but within
	// tokenedProcessReapAge (60m), so the sweep frees only the
	// untokened process.
...
func TestCloseBeforeSpawnAbortsTokenedStart(t *testing.T) {
	...
	// A start that loses the race to Close never runs the command.
	...
	// must be released for a retried start to reserve.
	m.mu.Lock()
