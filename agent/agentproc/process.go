...
type tokenEntry struct {
	...
	// created it: a different chat reusing the token conflicts
	// immediately instead of waiting behind a foreign dispatch.
	chatID string
	err    error
	done   chan struct{}
}
...
func (m *manager) start(ctx context.Context, req workspacesdk.StartProcessRequest, chatID string) (*process, bool, error) {
	...
	for {
		...
		// Another start owns this token but may not have published
		// its process yet (it is between reserve and spawn), so
		// there is nothing to return until it finishes. When it
		// already finished, done is closed and this select does
		// not block.
		select {
		case <-entry.done:
		case <-m.closeCh:
