...
func (c *Config) RefreshToken(ctx context.Context, db database.Store, externalAuthLink database.ExternalAuthLink) (database.ExternalAuthLink, error) {
	...
	ch := c.RefreshGroup.DoChan(key, func() (any, error) {
		...
		for {
			...
			case errors.Is(err, sql.ErrNoRows):
				...
				if !r.Wait(rctx) {
					return externalAuthLink, rctx.Err()
				}
			case err != nil:
	...
	refresh:
		...
		defer func() {
			releaseErr = db.ReleaseExternalAuthLinkRefreshLease(ctx, database.ReleaseExternalAuthLinkRefreshLeaseParams{
				ProviderID:            externalAuthLink.ProviderID,
				UserID:                externalAuthLink.UserID,
				RefreshLeaseExpiresAt: sql.NullTime{Time: lease, Valid: true},
			})
		...
		if dblink.OAuthRefreshToken != externalAuthLink.OAuthRefreshToken {
			return dblink, releaseErr
		}

		token, err := c.innerRefreshToken(rctx, db, dblink, lease)
	...
	if err != nil {
		...
		if isFailedRefresh(existingToken, err) {
			...
			dblink, updateErr := db.UpdateExternalAuthLink(ctx, database.UpdateExternalAuthLinkParams{
				...
				// The row will only update if we hold the current lease.
				RefreshLeaseExpiresAt: sql.NullTime{Time: lease, Valid: true},
			...
			if errors.Is(updateErr, sql.ErrNoRows) {
				// If we do not hold the lease, it means we hung long enough for
				// something else to take over.  We could try to check if there is a new
				// refresh token, but there is no guarantee it has been set yet, and our
				// context has expired anyway at this point, so just error.
				return externalAuthLink, InvalidTokenError(fmt.Sprintf("refresh token failed: %q", err.Error()))
			}
			if updateErr != nil {
