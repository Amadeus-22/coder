...
func TestPostTemplateByOrganization(t *testing.T) {
	...
	t.Run("AgentsAllowed", func(t *testing.T) {
		...
		for _, tc := range []struct {
			...
			{name: "Omitted", expect: true},
			{name: "True", allowed: ptr.Ref(true), expect: true},
			{name: "False", allowed: ptr.Ref(false), expect: false},
		} {
			t.Run(tc.name, func(t *testing.T) {
				...
				template := coderdtest.CreateTemplate(t, client, user.OrganizationID, version.ID, func(req *codersdk.CreateTemplateRequest) {
					req.AgentsAllowed = tc.allowed
				})
				assert.Equal(t, tc.expect, template.AgentsAllowed)
			})
		}
	})
