...
func TestExecuteToolClientToken(t *testing.T) {
	...
	identity := chattool.ExecuteIdentity{
		ChatID:             uuid.New(),
		AssistantMessageID: 42,
	}
	identityCtx := func(t *testing.T) context.Context {
		return chattool.WithExecuteIdentity(testutil.Context(t, testutil.WaitMedium), identity)
	}
	...
	t.Run("TokenDerivedFromIdentity", func(t *testing.T) {
		t.Parallel()
		ctrl := gomock.NewController(t)
		mockConn := agentconnmock.NewMockAgentConn(ctrl)
		...
		mockConn.EXPECT().
			...
			DoAndReturn(func(_ context.Context, req workspacesdk.StartProcessRequest) (workspacesdk.StartProcessResponse, error) {
				...
				return workspacesdk.StartProcessResponse{ID: "proc-1", Started: true, ClientToken: req.ClientToken}, nil
			}).
			Times(3)
		mockConn.EXPECT().
			ProcessOutput(gomock.Any(), "proc-1", gomock.Any()).
		...
		assert.Equal(t, tokens[0], tokens[1], "same identity and call ID must derive the same token")
		assert.NotEqual(t, tokens[0], tokens[2], "a different call ID must derive a different token")
	})

	t.Run("NoIdentityMeansNoToken", func(t *testing.T) {
		t.Parallel()
		ctrl := gomock.NewController(t)
		mockConn := agentconnmock.NewMockAgentConn(ctrl)
		...
		mockConn.EXPECT().
			StartProcess(gomock.Any(), gomock.Any()).
			DoAndReturn(func(_ context.Context, req workspacesdk.StartProcessRequest) (workspacesdk.StartProcessResponse, error) {
				assert.Empty(t, req.ClientToken)
	...
	t.Run("NoCallIDMeansNoToken", func(t *testing.T) {
		t.Parallel()
		ctrl := gomock.NewController(t)
		mockConn := agentconnmock.NewMockAgentConn(ctrl)
	...
	t.Run("ConflictNeverStartsASecondProcess", func(t *testing.T) {
		...
		mockConn.EXPECT().
			StartProcess(gomock.Any(), gomock.Any()).
			Return(workspacesdk.StartProcessResponse{}, codersdk.NewError(http.StatusConflict, codersdk.Response{
	...
	t.Run("TokenWaitAbortedRetriesAndAttaches", func(t *testing.T) {
		...
		waitAborted := codersdk.NewError(http.StatusConflict, codersdk.Response{
			Message: "Timed out waiting for the concurrent start that owns this client token.",
			Detail:  "wait for concurrent start with the same token: aborted waiting for the concurrent start owning this client token",
		})
		gomock.InOrder(
			mockConn.EXPECT().
				StartProcess(gomock.Any(), gomock.Any()).
				Return(workspacesdk.StartProcessResponse{}, waitAborted),
			mockConn.EXPECT().
				StartProcess(gomock.Any(), gomock.Any()).
	...
	t.Run("TokenWaitAbortedBudgetExpiresUnresolved", func(t *testing.T) {
		...
		mockConn.EXPECT().
			...
			Return(workspacesdk.StartProcessResponse{}, codersdk.NewError(http.StatusConflict, codersdk.Response{
				Message: "Timed out waiting for the concurrent start that owns this client token.",
				Detail:  "manager is closed: aborted waiting for the concurrent start owning this client token",
			}))
	...
	t.Run("BackgroundAttachReturnsSameHandle", func(t *testing.T) {
		...
		mockConn.EXPECT().
			...
			DoAndReturn(func(_ context.Context, req workspacesdk.StartProcessRequest) (workspacesdk.StartProcessResponse, error) {
				assert.True(t, req.Background)
				assert.NotEmpty(t, req.ClientToken)
				return workspacesdk.StartProcessResponse{ID: "proc-bg", Attached: true, ClientToken: req.ClientToken}, nil
			})
