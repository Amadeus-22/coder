...
func TestMigration000544TemplateAgentsAllowedBackfill(t *testing.T) {
	t.Parallel()

	sqlDB, ctx, orgID, userID, templateIDs := setupMigration000544Templates(t)
	upSQL, err := os.ReadFile("000544_template_agents_allowed.up.sql")
	require.NoError(t, err)
	downSQL, err := os.ReadFile("000544_template_agents_allowed.down.sql")
	require.NoError(t, err)
...
func setupMigration000544Templates(t *testing.T) (
	sqlDB *sql.DB,
	ctx context.Context,
	orgID uuid.UUID,
	userID uuid.UUID,
